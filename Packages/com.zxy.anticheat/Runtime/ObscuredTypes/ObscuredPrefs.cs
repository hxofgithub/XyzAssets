...
using System.Text;
using XyzAniCheat.Runtime.Common;
using UnityEngine;
using XyzAniCheat.Runtime.Utils;
...
namespace XyzAniCheat.Runtime.ObscuredTypes
{
    public static class ObscuredPrefs
    {
        ...
        public static void ForceLockToDeviceInit()
        {
            ...
            {
                Debug.LogWarning("ObscuredPrefs.ForceLockToDeviceInit() is called, but device ID is already obtained!");
            }
        }
        ...
        private static string GetEncryptedPrefsString(string key, string encryptedKey)
        {
            ...
            if (result == RAW_NOT_FOUND)
            {
                if (PlayerPrefs.HasKey(key))
                {
                    Debug.LogWarning("Are you trying to read regular PlayerPrefs data using ObscuredPrefs (key = " + key + ")?");
                }
            }
            return result;
        }
        ...
        private static string EncryptData(string key, byte[] cleanBytes, DataType type)
        {
            ...
            var dataHash = HashUtils.CalculateHash(cleanBytes, dataLength, 0);
        ...
        internal static byte[] DecryptData(string key, string encryptedInput)
        {
            ...
            var realDataHash = HashUtils.CalculateHash(cleanBytes, dataBytesLength, 0);
        ...
        private static uint CalculateChecksum(string input)
        {
            ...
            var hash = HashUtils.CalculateHash(inputBytes, inputBytes.Length, 0);
            return hash;
        }
